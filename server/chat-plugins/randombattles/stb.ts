...
function generateSTBMoveInfo(sigMove: Move, dex: ModdedDex) {
	let buf = ``;
...
function generateSTBAbilityInfo(set: STBSet, dex: ModdedDex, baseDex: ModdedDex) {
	let buf = ``;
	const customMegaAbilities = ['Sableye', 'Sceptile'];
...
function generateSTBInnateInfo(name: string, dex: ModdedDex, baseDex: ModdedDex) {
	let buf = ``;
...
function STBSets(target: string) {
	...
	let buf = '';
	const sets: string[] = [];
	for (const set in stbSets) {
		if (!set.startsWith(name)) continue;
		if (!stbSets[set].skip && set !== name) continue;
		sets.push(set);
	}
	for (const setName of sets) {
		const set = stbSets[setName];
		...
		// keys and Kennedy have an itemless forme change
			...
			buf += generateSTBPokemonInfo(`${set.species}-Gmax`, dex, baseDex);
		}
...
export const disabledSets = Chat.oldPlugins.stb?.disabledSets || [];

function enforceDisabledSets() {
	for (const process of Rooms.PM.processes) {
		process.getProcess().send(`EVAL\n\nConfig.disabledstbsets = ${JSON.stringify(disabledSets)}`);
	}
}
...
export const commands: Chat.ChatCommands = {
	...
	stbhelp: [
		`/stb [username] - Displays a user's Super Tiger Bros. set and custom features.`,
	],
	enablestbset: 'disablestbset',
	disablestbset(target, room, user, connection, cmd) {
		this.checkCan('rangeban');
		target = toID(target);
		if (!Object.keys(stbSets).map(toID).includes(target as ID)) {
			throw new Chat.ErrorMessage(`${target} has no STB set.`);
		}
		...
		if (cmd.startsWith('enable')) {
			...
			this.privateGlobalModAction(`${user.name} enabled ${target}'s STB set.`);
		} else {
			if (disableIdx > -1) {
				throw new Chat.ErrorMessage(`That set is already disabled.`);
			}
			disabledSets.push(target);
			this.privateGlobalModAction(`${user.name} disabled the STB set for ${target}`);
		}
		enforceDisabledSets();
	},
};
