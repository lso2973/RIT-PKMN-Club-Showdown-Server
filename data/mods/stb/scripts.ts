export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
		...
		useMoveInner(moveOrMoveName, pokemon, target, sourceEffect, zMove, maxMove) {
			...
			if ((move.forceSwitch || move.selfSwitch) && this.battle.getAllActive().some(x => x.hasAbility('skilldrain'))) {
				this.battle.hint(`Self-switching and force switch moves don't trigger when a Pokemon with Skill Drain is active.`);
			}
	...
	pokemon: {
		isGrounded(negateImmunity) {
			...
			if (this.hasAbility(['levitate', 'candlewax'])) return false;
