...
export const Moves: {[k: string]: ModdedMoveData} = {
	...
	*/
	...
	stareater: {
		...
		basePower: 50,
		category: "Physical",
		desc: "Restores HP equal to the average of the target's Atk and SpAtk stats, and lowers whichever one is higher by one stage. Additionally, if this move runs out of pp the user faints and does damage to the opponent equal to the user's current HP.",
		shortDesc: "Heals by target's (Atk+SpA)/2, -1 to opponent's higher stat, Final Gambit at 0pp",
		name: "Star Eater",
		gen: 8,
	...
	boneappetit: {
		...
		onHit(pokemon, move) {
			...
			if (!this.singleEvent('TakeItem', item, pokemon.itemState, pokemon, move, item)) return false;
			if (!item.fling) return false;
	...
	doublesspreadreduction: {
		...
		condition: {
			...
			onResidualOrder: 25,
			onEnd() {
				this.add('-fieldend', 'move: Doubles Spread Reduction', '[of] ' + this.effectState.source);
			},
		},
		target: "all",
		type: "Psychic",
